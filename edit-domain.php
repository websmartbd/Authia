...
<head>
  ...
  <title>Edit Domain - Authia</title>
      ...
      <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 pt-20 md:p-8 flex justify-center items-start">
        <div class="w-full max-w-3xl">
            ...
            <div class="hidden md:flex items-center justify-between mb-8">
                <div>
                   <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Domain</h1> 
                   <p class="text-sm text-slate-500 dark:text-slate-400">Update licensing details.</p>
                </div>
            ...
            <form method="POST" class="bg-white dark:bg-slate-900 shadow-sm rounded-2xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="space-y-2">
                  <label class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Expiry Date</label>
                ...
                <div class="space-y-2">
                  ...
                  <select name="delete" id="delete" 
                    class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all">
                      <option value="yes" <?php echo ($domain_data['delete'] == 'yes') ? 'selected' : ''; ?>>Flagged</option>
                      <option value="no" <?php echo ($domain_data['delete'] == 'no') ? 'selected' : ''; ?>>None</option>
                  </select>
                </div>
              </div>
              ...
              <div class="space-y-2">
                ...
                <textarea name="message" id="message" rows="4"
                  ...
                  placeholder="Additional context..."><?php echo htmlspecialchars($domain_data['message']); ?></textarea>
              </div>
              ...
              <div class="flex flex-col sm:flex-row justify-end pt-4 border-t border-slate-100 dark:border-slate-800 gap-3">
                ...
                <button type="submit" name="edit" class="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-lg shadow-indigo-900/20 transition-all transform hover:-translate-y-0.5 active:translate-y-0 text-center">
                  Save Changes
                </button>
              </div>
            </form>
